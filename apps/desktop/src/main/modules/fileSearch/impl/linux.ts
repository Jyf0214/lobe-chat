...
/**
 * Linux file search implementation using fast-glob
 * Falls back to pure Node.js implementation without external tool dependencies
 */
export class LinuxSearchServiceImpl extends FileSearchImpl {
  ...
  async search(options: SearchOptions): Promise<FileResult[]> {
    ...
    logger.debug('Starting Linux file search', { keywords: options.keywords, searchDir });
    ...
    try {
      ...
      logger.error('Linux search failed:', error);
