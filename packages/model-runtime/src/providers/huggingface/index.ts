  ...
  models: async () => {
    ...
    const formattedModels = modelList
      .map((model) => {
        ...
        // Select provider info priority: is_model_author > info completeness > default first
        const mainProvider =
        ...
        // Multiple provider fallback strategy: get from main provider first, check others if missing
        const getFieldFromProviders = (field: keyof typeof mainProvider) => {
          const value = mainProvider[field];
          if (value !== undefined && value !== null) {
            return value;
          }
          // Traverse other providers when missing
        ...
        // displayName uses id with left side of slash removed (e.g. 'zai-org/GLM-4.6' -> 'GLM-4.6')
        const displayName =
