...
export class LobeOllamaAI implements LobeRuntimeAI {
  ...
  async pullModel(params: PullModelParams, options?: ModelRequestOptions): Promise<Response> {
    ...
    const signal = options?.signal; // Get the passed AbortSignal
    ...
    // If there is an AbortSignal, listen for abort event
    // Use { once: true } to ensure listener is only triggered once
    signal?.addEventListener('abort', abortOllama, { once: true });

    try {
      // Get the Ollama pull iterator
      ...
      // Use the dedicated model pull stream conversion method
      const progressStream = createModelPullStream(iterable, model, {
        onCancel: () => {
          // When stream is cancelled, call abortOllama
          // Remove signal listener to avoid duplicate calls (if abortOllama is not idempotent)
      ...
      // If error occurs during client.pull call or stream creation initial phase, need to remove listener
      signal?.removeEventListener('abort', abortOllama);
      ...
      // Check if it's a cancel operation
