...
import { Activity, ArrowRight, Award, BarChart3, Database, FlaskConical, Gauge, LoaderPinwheel, Play, Server, Target, TrendingUp, Trophy, Upload, User, Volleyball, Zap } from 'lucide-react';
...
const BenchmarkCard = memo<BenchmarkCardProps>(
  ({ id, name, description, testCaseCount, recentRuns, runCount = 0, bestScore, source, tags, datasetCount = 0 }) => {
      ...
      <Flexbox className={styles.card} gap={12} justify="space-between">
                ...
                {bestScore !== undefined && (
                  <>
                    ...
                    <span>
                      ...
                      <span style={{ color: 'var(--ant-color-text)', fontFamily: 'monospace', fontWeight: 600 }}>
                        {bestScore.toFixed(1)}
                      </span>
                    </span>
                  </>
        ...
        {tags && tags.length > 0 && (
          <Flexbox gap={4} horizontal style={{ flexWrap: 'wrap' }}>
            ...
            {tags.length > 4 && (
              <Tag style={{ fontSize: 10 }}>+{tags.length - 4}</Tag>
            )}
          </Flexbox>
        )}
        </Flexbox>
        ...
        {!hasDatasets ? (
          <div className={styles.emptyBox}>
            <Icon icon={Database} size={24} style={{ color: 'var(--ant-color-text-quaternary)', marginBottom: 8 }} />
          ...
          <Flexbox gap={8}>
            ...
            {displayRuns.length > 0 ? (
              ...
              <div className={styles.emptyBox}>
                <Icon icon={FlaskConical} size={24} style={{ color: 'var(--ant-color-text-quaternary)', marginBottom: 8 }} />
                <p style={{ color: 'var(--ant-color-text-tertiary)', fontSize: 13, margin: '0 0 4px' }}>
